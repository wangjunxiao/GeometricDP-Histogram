...
from mechanisms.utils import Budget, BudgetError
from mechanisms.validation import check_epsilon_delta
...
class BudgetAccountant:
    ...
    Other functions can make use of a BudgetAccountant in three different ways (see examples for more details):
        - Passed as an ``accountant`` parameter to the function (e.g., ``mean(..., accountant=acc)``)
        - Set as the default using the ``set_default()`` method (all subsequent functions will use the
    ...
    Examples
    --------
    A ``BudgetAccountant`` is typically passed to other functions as an ``accountant`` parameter.  If ``epsilon``
    ...
    >>> X = random(100)
    >>> acc = BudgetAccountant(epsilon=1.5, delta=0)
    >>> mean(X, bounds=(0, 1), accountant=acc)
    0.4547006207923884
    >>> acc.total()
    (epsilon=1.0, delta=0)
    >>> std(X, bounds=(0, 1), epsilon=0.25, accountant=acc)
    0.2630216611181259
    >>> acc.total()
    (epsilon=1.25, delta=0)
    >>> acc2 = BudgetAccountant() # infinite budget
    >>> first_half = mean(X[:50], epsilon=0.25, bounds=(0, 1), accountant=acc2)
    >>> last_half = mean(X[50:], epsilon=0.25, bounds=(0, 1), accountant=acc2)
    >>> acc2.total()
    (epsilon=0.5, delta=0)
    >>> acc2.remaining()
    (epsilon=inf, delta=1.0)
    >>> acc3 = BudgetAccountant(slack=1e-3)
    >>> for i in range(20):
    ...     mean(X, epsilon=0.05, bounds=(0, 1), accountant=acc3)
    >>> acc3.total() # Slack has reduced the epsilon spend by almost 25%
    (epsilon=0.7613352285668463, delta=0.001)
    Using ``set_default()``, an accountant is used by default in all functions in that script.  Accountants
    ...
    >>> acc4 = BudgetAccountant()
    >>> acc4.set_default()
    BudgetAccountant()
    >>> Y = random((100, 2)) - 0.5
    >>> clf = PCA(1, centered=True, data_norm=1.4)
    >>> clf.fit(Y)
    ...
    >>> acc4.total()
    (epsilon=1.0, delta=0)
    >>> with BudgetAccountant() as acc5:
    ...     mean(Y, bounds=(0, 1), epsilon=1/3)
    >>> acc5.total()
    ...
    def set_default(self):
        """Sets the current accountant to be the default when running functions and queries.
        Returns
        -------
        self : BudgetAccountant
        """
