...
def warn_unused_args(args):
    """Warn the user about supplying unused `args` to a model.
    ...
    for arg in args:
        warnings.warn(f"Parameter '{arg}' is not functional.  Remove this parameter to suppress this "
...
class CompatibilityWarning(RuntimeWarning):
    """Custom warning to capture inherited class arguments that are not compatible.
    ...
    For example, this warning may occur when the user:
        - passes a parameter value that is not compatible (e.g., `solver='liblinear'` to
          :class:`.LogisticRegression`)
        - specifies a non-default value for a parameter that is ignored (e.g., `intercept_scaling=0.5`
